...
class PromotionRepository {
  ...
  Future<List<Promotion>> getPromotionsByType(DiscountType type) async {
    await Future.delayed(const Duration(milliseconds: 300));
    return mockPromotions.where((promotion) => promotion.discountType == type).toList();
  }
  ...
  Future<List<Promotion>> getFixedAmountPromotions() async {
    await Future.delayed(const Duration(milliseconds: 300));
    return mockPromotions
        .where((promotion) => promotion.discountType == DiscountType.FixedAmount)
        .toList();
  }
  ...
  Future<List<Promotion>> getPromotionsWithMinimumDiscount(double minValue) async {
    await Future.delayed(const Duration(milliseconds: 300));
    return mockPromotions
  ...
  Future<Map<String, dynamic>> getPromotionStatistics() async {
    ...
    final averageDiscountValue = mockPromotions
        .map((p) => p.discountValue)
        .reduce((a, b) => a + b) / totalPromotions;
