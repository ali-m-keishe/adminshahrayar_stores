...
class MenuRepository {
  ...
  Future<List<Category>> getCategories() async {
    try {
      ...
      return [
        Category(id: 1, name: 'Main Course', image: '', createdAt: DateTime.now()),
        Category(id: 2, name: 'Appetizer', image: '', createdAt: DateTime.now()),
  ...
  Future<List<MenuItem>> getMenuItemsSortedByPrice({bool ascending = true}) async {
  ...
  Future<Map<String, dynamic>> getMenuStatistics() async {
    ...
    final categories = mockMenuItems.map((item) => item.categoryId).toSet().length;
    final totalItems = mockMenuItems.length;
    final averagePrice = mockMenuItems
        .map((item) => item.price)
        .reduce((a, b) => a + b) / totalItems;
    
    final priceRange = {
      'min': mockMenuItems.map((item) => item.price).reduce((a, b) => a < b ? a : b),
      'max': mockMenuItems.map((item) => item.price).reduce((a, b) => a > b ? a : b),
    };
