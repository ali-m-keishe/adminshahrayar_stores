...
class StaffRepository {
  ...
  Future<List<StaffMember>> getDeliveryStaff() async {
    await Future.delayed(const Duration(milliseconds: 300));
    return mockStaff
        .where((staff) => staff.role == StaffRole.Delivery)
        .toList();
  }
  ...
  // Update staff status
  Future<StaffMember> updateStaffStatus(
      String staffId, StaffStatus status) async {
    await Future.delayed(const Duration(milliseconds: 500));
    final staff = await getStaffById(staffId);
    if (staff == null) {
      throw Exception('Staff member not found');
  ...
  Future<Map<String, dynamic>> getStaffStatistics() async {
    ...
    final activeStaff =
        mockStaff.where((s) => s.status == StaffStatus.Active).length;
  ...
  Future<List<StaffMember>> getAvailableStaffForRole(StaffRole role) async {
    ...
    return mockStaff
        .where(
            (staff) => staff.role == role && staff.status == StaffStatus.Active)
        .toList();
  }
}
